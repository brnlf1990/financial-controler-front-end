...
function MonthCard({month, currentYear, prevMonth, nextMonth, openModal}) {
    ...
    return ( 
            ...
            <div className="month-card">
                ...
                <button className="month-card__prev-button" onClick={prevMonth}>Anterior </button>
                <div className="month-card__title">{month.format("MMMM YYYY")}</div>
                
                <button className="month-card__next-button" onClick={nextMonth}>Próximo</button>
                </div>
                <div className="month-card__week-days">{weekDays.map((week) =>(
                    <div className="week_day">{week} </div>
                ))}</div>
                {
                    calendar.map((week, weekIndex) => (
                        <div className="week" key={weekIndex}> 
                        {week.map((day)=>(
                            <DayCard  key={day.format("DD-MM-YYYY")} 
                            day={day}
                            month={month}
                            year={currentYear}
                            openModal={openModal}
